...
class Catch(dm_env.Environment):
    ...
    def step(self, action: int) -> dm_env.TimeStep:
        ...
        if self._ball_y == self._paddle_y:
            ...
            print("Ball caught") if reward ==1 else print("ball dropped")
            self._reset_next_step = True
            return dm_env.termination(reward=reward, observation=self._observation())
        else:
            return dm_env.transition(reward=0., observation=self._observation(), discount=0.7)
    ...
    def _observation(self) -> np.ndarray:
        ...
        return self._ball_x, self._ball_y, self._paddle_x, self._paddle_y
